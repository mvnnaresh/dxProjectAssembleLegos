...
ui(new Ui::MainWindow)
{
	...
	connect(initButton, &QPushButton::clicked, this, [this]()
	{
		if (!mDemo)
		{
			...
			connect(mDemo.get(), &demo::ctrlTargetsReady, mSim, &dxMuJoCoRobotSimulator::setCtrlTargets, Qt::QueuedConnection);
			connect(mDemo.get(), &demo::jointPositionsReady, mSim, &dxMuJoCoRobotSimulator::setJointPositions, Qt::QueuedConnection);
			connect(mDemo.get(), &demo::ctrlTargetsFromJointsReady, mSim, &dxMuJoCoRobotSimulator::setCtrlTargetsFromJointPositions, Qt::QueuedConnection);
			connect(mDemo.get(), &demo::drawTrajectory, mViewer, &dxMuJoCoRobotViewer::drawTrajectory);
			connect(mDemo.get(), &demo::drawFrames, mViewer, &dxMuJoCoRobotViewer::drawFrames);
			connect(mDemo.get(), &demo::closeGripperRequested, mSim, &dxMuJoCoRobotSimulator::closeGripper, Qt::QueuedConnection);
			connect(mDemo.get(), &demo::gripperPositionRequested, mSim, &dxMuJoCoRobotSimulator::setGripperPosition, Qt::QueuedConnection);
		...
		QMetaObject::invokeMethod(mSim, "loadModel", Qt::QueuedConnection, Q_ARG(QString, QString::fromStdString(mModelPath)));
