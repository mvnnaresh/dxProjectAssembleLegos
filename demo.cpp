...
void demo::testLegoAssembly()
{
                            ...
                            double placeCenterZ) -> bool
    {
        ...
        if (!sendRobotTo(startPose, pregraspPose, 20))
        {
            return false;
        }
        if (!sendRobotTo(pregraspPose, graspPose, 20))
        {
            return false;
        }
        ...
        if (!sendRobotTo(graspPose, liftPose, 20))
        {
            return false;
        }
        if (!sendRobotTo(liftPose, preplacePose, 20))
        {
            return false;
        }
        if (!sendRobotTo(preplacePose, placePose, 20))
        {
            return false;
        }
        if (seatDepth > 0.0)
        {
            if (!sendRobotTo(placePose, seatPose, 20))
            {
                return false;
            }
        }
        ...
        sendRobotTo(placePose, preplacePose, 20);
        return true;
    };
    ...
    const double blueX = redPose[0] - 0.04;
    const double blueY = redPose[1];
    const double blueCenterZ = blueInfo.centerZ;
